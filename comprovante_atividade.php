...
if (isset($_GET['id_atividade_docente'])) {
    $id_atividade_docente = $_GET['id_atividade_docente'];
} else if (isset($_POST['id_atividade_docente'])) { 
    $id_atividade_docente = $_POST['id_atividade_docente'];
} else {
    die('Operação inesperada!!! - Entre em contato com o administrador do sistema!');
}
...
$result_atividade_docente = $atividade_docenteM->getUltimaAtividadeDdocente($id_atividade_docente);
$linha_atividade_docente = mysqli_fetch_assoc($result_atividade_docente);

$comprovanteM = new comprovanteModel();
$result = $comprovanteM->getComprovanteAtividade($id_atividade_docente);
...
if (isset($_POST['descricao'])) {
    if ($acao == 'inserir') {
        $res = $comprovanteM->inserirComprovanteAtividade($_POST);
        if ($res) {
            $id_comprovante = $res;
        ...
        $_POST['situacao'] = $linha_atividade_docente['situacao'];
        $_POST['etapa'] = $linha_atividade_docente['etapa'];
        $res = $comprovanteM->atualizarComprovanteAtividade($_POST);
        if ($res) {
            $descricao = $_POST['descricao'];
            $msg = '<div class="alert alert-success">';
            ...
            $msg .= '<div class="alert alert-danger">';
            $msg .= 'Erro ao atualizar - Contactar o administrador do sistema';
            $msg .= '</div>';
        }        
    }
}
...
<html lang="pt-BR">
    ...
    <body>
        ...
        <form id="formulario" method="post" action="comprovante_atividade.php" enctype="multipart/form-data">
            ...
            <input type="hidden" name="id_atividade_docente" id="id_atividade_docente" value="<?=$id_atividade_docente?>">

            <div class="container">
                <div class="col-md-12">
                    <div class="panel panel-default">
                        <div class="panel-heading text-center">Adicionar comprovante da atividade</div>
                        <div class="panel-body">    
                            ...
                            <div class="alert alert-warning" style="text-align:justify">
                                ...
                                <ul>
                                    <li>Fazer o upload do arquivo de comprovação da execução da atividade.</li>
                                    <li>O comprovante deve estar em formato PDF.</li>
                                    <li>Se neste item houver um grupo de atividade do mesmo tipo executadas, os comprovantes devem ser juntados em um único PDF!</li>
                                </ul>
                                <br>
                                <b>Atividade:</b> <?=$linha_atividade_docente['atividade']?><br>
                                <b>Descricação:</b> <?=$linha_atividade_docente['descricao']?><br>
                            </div>
