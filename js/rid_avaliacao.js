var classe = 'rid_avaliacaoController';
...
function avaliar_rid() {

    $('#metodo').val('avaliar_rid');
    $('#etapa').val('RID');
...
function carregar() {
    ...
    $.ajax({
        ...
        $('#horas_executadas').val(json.horas_executadas);

        $('#descricao').attr('readonly', false);
        $('#horas_planejadas').attr('readonly', true);        
        $('#horas_executadas').attr('readonly', false); 
        $('#observacao').val('');
        $('#situacao').val('');
...
function abrirModal(modal, metodo, id_atividade_docente, id_tipo_atividade) {
    ...
    if (metodo == 'visualizar_atividade') {
        ...
        $.ajax({
            ...
            $('#div_historico_atividade2').html(tabela);
        });            
    } else {
        //console.log('teste');
...
function listar(msg) {
    ...
    $.ajax({
        ...
        var json = JSON.parse(resposta);
        $('#rid_professor').html(json.tabela);
...
function habilitar_campos(obj){
    if (obj.value == 'APROVADA COM ALTERAÇÃO') {
        ...
        $('#horas_planejadas').attr('readonly', true);
        $('#horas_executadas').attr('readonly', true);
    }
}

function habilita_correcao(obj) {
    if (obj.value == 'REPROVADO') {
        $('#rid_correcao_fim').attr('readonly', true);
    } else {
        $('#rid_correcao_fim').attr('readonly', false);
    }
}
...
function imprimir() {
    ...
    $.ajax({
        ...
        var win = window.open("rid_professor.html", "", "width=1500, height=760");
        var cabecalho = "";
        var titulo = "";
        var fim = "";
